...
### **Раскраска элементов по условию**
...
Простой и в то же время очень "вкусный" критерий (хотя, конечно, нужны базовые знания C#)

1. В разметку добавляем привязку для фона
...
2. И в классе "продукт" реализуем геттер для этого свойства
    ...
    public string BackgroundColor 
    {
        get 
        {
            // виртуальное свойство ProductSale содержит коллекцию продаж нашего товара
            if(ProductSale.Count==0)
                return "#faa";

            // находим последнюю дату продажи    
            var LastSale = ProductSale.Max(ps => ps.SaleDate);

            // вычитаем из текущей даты
            var TimeDelta = DateTime.Now - LastSale;

            // при вычитании дат получается структура DateSpan, у которой есть нужные данные
            if(TimeDelta.TotalDays>30)
                return "#faa";

            // если продажи были, то оставляем белый фон    
            return "#fff";
        }
    }
    ```
...
**Во-первых**, в задании ошибка. Явно сказано, что цена продукта должна считаться по материалам. Но в этом разделе нужно менять минимальную цену для агента, хотя она нигде не показывается (возможно надо выводить максимальную из этих двух сумм).  

**Во-вторых**, стоит это маразм около 1,5 баллов и с этим точно не стоит связываться.